...
export class FabricGateway {
  ...
  async initialize() {
    ...
    console.log(JSON.stringify(this.fabricConfig, undefined, 2));
    const explorerAdminId = this.fabricConfig.getAdminUser();
    console.log('explorerAdminId = ' + explorerAdminId);
    if (!explorerAdminId) {
    ...
    try {
      ...
      if (identity) {
        ...
        console.log('enrollCaIdentity');
        await this.enrollCaIdentity(
          explorerAdminId,
          this.fabricConfig.getAdminPassword()
        );
      } else {
        console.log('Identity credentials to be stored in the wallet');
        /*
      ...
      console.log(JSON.stringify(connectionOptions, undefined, 2));
  ...
  async enrollUserIdentity(userName, signedCertPem, adminPrivateKeyPem) {
    logger.info('start enrollUserIdentity: userName :', userName);
    const identity = {
      credentials: {
  ...
  async enrollCaIdentity(id, secret) {
    logger.info(`start enrollCaIdentity for ${id} and ${secret}`);
    ...
    try {
      ...
      logger.info(`try2`);
      logger.info(`caConfig: ` + JSON.stringify(caConfig));

      let tlsCACert;
      if (caConfig.serverCertPath) {
        tlsCACert = fs.readFileSync(caConfig.serverCertPath, 'utf8');
      } else {
        tlsCACert = caConfig.tlsCACerts.pem;
      }

      logger.info(`tlsCACert: ` + tlsCACert);
      logger.info(`caConfig.caURL[0]: ` + caConfig.caURL[0]);
      logger.info(`trustedRoots: ` + tlsCACert);
      logger.info(`caConfig.caName: ` + caConfig.caName);
      ...
      logger.info(
        `enrolling user:` +
          ...
          ', password: ' +
          this.fabricConfig.getCaAdminPassword()
      );
  ...
  async getDiscoveryResult(channelName) {
    try {
      ...
      const tlsCert = fs
        .readFileSync(
          '/home/mei/go/src/github.com/hyperledger/fabric/scripts/fabric-samples/test-network/organizations/peerOrganizations/org1.example.com/users/Admin@org1.example.com/msp/signcerts/cert.pem'
        )
        .toString();
      const tlsKey = fs
        .readFileSync(
          '/home/mei/go/src/github.com/hyperledger/fabric/scripts/fabric-samples/test-network/organizations/peerOrganizations/org1.example.com/users/Admin@org1.example.com/msp/keystore/12828180b3a900b1c93ce6e8432be43406eefd24ec51f3f282200eaa41f9c973_sk'
        )
        .toString();
      ...
      logger.info('client._tls_mutual');
      logger.info(client._tls_mutual);
      ...
      for (const peer of this.config.organizations[mspID].peers) {
        ...
        logger.info(
          `peer options for client ${peer}`,
          JSON.stringify(this.config.peers[peer])
        );
        logger.info(`grpcOpt for client ${peer}`, JSON.stringify(grpcOpt));
