...
export class FabricConfig {
	...
	isFabricCaEnabled() {
		...
		logger.info("reading org: " + JSON.stringify(org));
		const caArray = this.config.organizations[org].certificateAuthorities;
		logger.info("caArray:" + JSON.stringify(caArray));
	 ...
	 */
	getMspId() {
		logger.info("org: " + this.getOrganization());
		logger.info(
			"this.config.organizations: " + JSON.stringify(this.config.organizations)
		);
		const organization = this.config.organizations[this.getOrganization()];
		...
		if (this.config.certificateAuthorities) {
			for (const x in this.config.certificateAuthorities) {
				if (this.config.certificateAuthorities[x].tlsCACerts) {
					...
					tlsCACerts = this.config.certificateAuthorities[x].tlsCACerts;
				}
				if (this.config.certificateAuthorities[x].url) {
					caURL.push(this.config.certificateAuthorities[x].url);
				}
				caName = this.config.certificateAuthorities[x].caName;
			}
		}
		return { caURL, serverCertPath, tlsCACerts, caName };
	}
